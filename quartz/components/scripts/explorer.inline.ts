...
async function setupExplorer(currentSlug: FullSlug) {
  ...
  for (const explorer of allExplorers) {
    ...
    // Restore explorer scrollTop position when available.
    const scrollTop = sessionStorage.getItem("explorerScrollTop")
    if (scrollTop && explorerContent instanceof HTMLElement) {
      const parsed = parseInt(scrollTop)
      if (!Number.isNaN(parsed)) {
        explorerContent.scrollTop = parsed
      }
    }
...
document.addEventListener("prenav", async () => {
  ...
  if (!(explorerContent instanceof HTMLElement)) return
